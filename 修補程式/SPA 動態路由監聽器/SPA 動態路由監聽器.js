 ...
 */
  ...
  class DynamicRouteHandler {
     ...
     * @param {RegExp[]} options.matchPatterns - 要匹配的網址正則陣列（必填）
     * @param {Function} [options.onEnter] - 進入匹配頁面時呼叫
     * @param {Function} [options.onLeave] - 離開匹配頁面時呼叫
    ...
    #checkAndTrigger() {
      ...
      if (isMatch && !this.isActive) {
        ...
        this._log('進入匹配頁面', currentUrl);
      } else if (!isMatch && this.isActive) {
        this.isActive = false;
        this.options.onLeave();
        this._log('離開匹配頁面', currentUrl);
      }
    }
  }

  window.DynamicRouteHandler = DynamicRouteHandler;
  if(window.DynamicRouteHandler) console.log('[DynamicRouteHandler] v1.0.1 已載入');
