...
class ArtistUpdateCatcher {
    ...
    async fetchUpdateArticles(url) {
        ...
        if (isDiscord) {
            ...
            creatorPostsApi ='https://pawchive.pw/api/v1' + cleanUrl;
            creatorInfoApi = 'https://pawchive.pw/api/v1' + cleanUrl + '/profile';
        }else{
        ...
        for (let post of newerPosts) {
            try {
                ...
                const imgSrc = isPawchive ? `https://img.pawchive.pw/thumbnail/data/${filePath}` : `${filePath}`;
